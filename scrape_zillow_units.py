...
async def scrape_complex(page, url):
    ...
    try:
        ...
        for i in range(5):
            ...
            if await page.locator("[data-testid='unit-table-body']").count() > 0:
                break

        # Step 2: ensure table is visible
        if await page.locator("[data-testid='unit-table-body']").count() == 0:
            print("⚠️ No unit table detected using ID. Attempting visual fallback...")
            # We will still proceed, as the fallback parser might find the table rows anyway
        else:
        ...
        expanded = 0
        while True:
            # Check within the wrapper, regardless of tag name
            show_more_btn = page.locator(
                "[data-testid='unit-table-body'] tr:has-text('Show') >> text=more units"
            )
...
def parse_units(html, url, total_units):
    ...
    # Updated: Look for the rows within any element having data-testid="unit-table-body"
    rows = soup.select('[data-testid="unit-table-body"] tr')
    
    # Fallback: if Zillow renames 'unit-table-body' entirely, scan all table rows for rent prices
    if not rows:
        all_trs = soup.find_all("tr")
        for tr in all_trs:
            if re.search(r'\$\d[,\d]*', tr.get_text()) and len(tr.find_all("td", recursive=False)) >= 4:
                rows.append(tr)
    ...
    for row in rows:
        ...
        # If rent doesn't look like a price, skip (could be a table header)
        if not re.search(r'\$\d', rent_cell):
            continue
    ...
    async with async_playwright() as p:
        ...
        for i, prop_data in enumerate(properties_to_scrape):
            ...
            if html:
                ...
                print(f"📊 Found {len(units)} units successfully.")
